$(document).ready(function() {
    ...
    var settings = {
        "url": "https://shopee.co.id/api/v4/search/search_items?by=relevancy&keyword=case%20iphone%2011&limit=60&newest=0&order=desc&page_type=search&scenario=PAGE_GLOBAL_SEARCH&version=2",
        "method": "GET",
        "timeout": 0,
        "headers": {
          "Cookie": "REC_T_ID=1c21f7e7-4549-11ed-8202-2abea087b1fa; SPC_F=S4dtgwn15u2BvNiNovcsXV7X7aDIsN1y; SPC_R_T_ID=1fWiT4DtT6kjv400v1YgaubdQzPCfQnXpmzboZmCgPxc3nanKhfTEx6i12hLCGxxQcOOujJmIdymOwIO7THr68hyKxomLBKUYEKDFaj9t0rqrOjDzw4t++iQMrIaZ9mr/WJepo1/HnoXLqRlQ2WXPsWwEBiD29/DnGej892wWZc=; SPC_R_T_IV=WnVOaU1iaFlwTXJ5d1lBRg==; SPC_SI=bVY9YwAAAABNNERDV3daOJxgGgAAAAAAcExOTmd4MG8=; SPC_T_ID=1fWiT4DtT6kjv400v1YgaubdQzPCfQnXpmzboZmCgPxc3nanKhfTEx6i12hLCGxxQcOOujJmIdymOwIO7THr68hyKxomLBKUYEKDFaj9t0rqrOjDzw4t++iQMrIaZ9mr/WJepo1/HnoXLqRlQ2WXPsWwEBiD29/DnGej892wWZc=; SPC_T_IV=WnVOaU1iaFlwTXJ5d1lBRg=="
        },
    };
    ...
    $('#generate_no_surat').on('click',function(e) {
        ...
        $.ajax({
            type:'GET',
            url:'/form_perjalanan_dinas/create',
    ...
    function Generate_PDF(data_perjalanan_dinas){
        ...
        if(provider == 'Lainnya'){
            provider = $('#other_provider').val();
        }
        var tujuan = $('#tujuan').val();
        var durasi_hari = $('#durasi_hari').val();
        var durasi_malam = $('#durasi_malam').val();
        var lokasi_keberangkatan = $('#lokasi_keberangkatan').val();
        var tgl_keberangkatan = $('#tgl_keberangkatan').val();
        var transportasi_keberangkatan = $('#transportasi_keberangkatan').val();
        var lokasi_kedatangan = $('#lokasi_kedatangan').val();
        var tgl_kedatangan = $('#tgl_kedatangan').val();
        var transportasi_kedatangan = $('#transportasi_kedatangan').val();
        var area = $('#area').val();
        var konsumsi = $('#konsumsi').val();
        var uang_saku = formatRupiah(data_perjalanan_dinas.uang_saku.toString());
        var uang_transportasi = formatRupiah(data_perjalanan_dinas.uang_transportasi.toString());
        var durasi = data_perjalanan_dinas.durasi;
        var catatan_tambahan = $('#catatan_tambahan').val();
        ...
        var yPos = 45;
        doc.text("FORM PENGAJUAN PERJALANAN DINAS", 45, yPos,);
        yPos += 6;
        doc.setFontSize(13);
        ...
        doc.setFontType('bold');
        doc.text("Identitas Pemohon", 30, yPos);
        yPos += 5;

        doc.setFontType('regular');
        doc.text("Nama", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(nama, 85, yPos);
        yPos += 5;

        doc.text("NIK", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(nik, 85, yPos);
        yPos += 5;

        doc.text("Jabatan", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(jabatan, 85, yPos);
        yPos += 5;

        doc.text("Departemen", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(departemen, 85, yPos);
        yPos += 5;

        doc.text("Nomor Rekening", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(provider, 85, yPos);
        doc.text(norek, 95, yPos);

        doc.setFontType('bold');
        yPos += 7;
        doc.text("B.", 20, yPos);
        doc.text("Informasi Perjalanan", 30, yPos);
        yPos += 7;

        doc.setFontType('regular');
        // doc.text("Tujuan", 35, yPos);
        // doc.text(":", 80, yPos);
        ...
        doc.text("Area Dinas", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(area, 85, yPos);
        yPos += 5;

        doc.text("Durasi", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(durasi+' hari', 85, yPos);
        yPos += 5;

        doc.text("Jmlh. Konsumsi", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text(konsumsi+' x', 85, yPos);
        yPos += 5;

        doc.text("Uang Konsumsi", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text('Rp.'+uang_saku+',-', 85, yPos);

        yPos += 5;
        doc.text("Uang Transportasi", 35, yPos);
        doc.text(":", 80, yPos);
        doc.text('Rp.'+uang_transportasi+',-', 85, yPos);

        // doc.text("Durasi Perjalanan", 35, yPos);
        // doc.text(":", 80, yPos);
        ...
        doc.text(":", 80, yPos);
        doc.text(lokasi_keberangkatan, 85, yPos);
        yPos += 5;
        ...
        doc.text("Transportasi Keberangkatan", 35, yPos);
        doc.text(":", 80, yPos);
        ...
        yPos += 7;
        doc.text("E.", 20, yPos);
        doc.text("Catatan Tambahan", 30, yPos);
        yPos += 7;

        var splittext = doc.splitTextToSize(catatan_tambahan, 155);
        doc.setFontType('regular');
        for(var i = 0; i<splittext.length; i++){
            doc.text(35, yPos, splittext[i]);
            yPos += 5;
        }
        ...
        // Form Pembatalan perjalanan Dinas
        ...
        doc.setFontType('bold');
        var yPos = 45;
        doc.text("SURAT PERNYATAAN PEMBATALAN PERJALANAN DINAS", 28, yPos,);
        yPos += 6;
        doc.setFontSize(13);
        doc.text("Nomor : UNI/GA/PPD/            /     /     /   ", 63, yPos,);
        doc.setFontSize(10);
        yPos += 10;
        ...
        doc.setProperties({ title: no_surat });

        var formData = new FormData();
        formData.append('id_perjalanan_dinas' , data_perjalanan_dinas['id']);
        formData.append('nama_file' , no_surat);
        formData.append('file_pengajuan' , doc.output('blob'));
        $.ajax('/save_data_perjalanan_dinas',
        {
            method: 'POST',
            data: formData,
            processData: false,
    ...
    function simpan_to_trello(data_perjalanan_dinas){
        ...
        // var idList = '62a95b6de5aa40089754ff41';
        var idList = '62b14978eb936f7458edf968';
        var key = '6a708346ca5432fc510200f0b0f555c1';
        var token = 'cd02196880c9446c6664de3374d091646ddfc5b87c5804430d149fd99dc84e2d';
        ...
        var desc =  'No. Surat : '+ no_surat + '\n' +
                    ...
                    'Nomor Rekening : '+provider+' '+ norek+ '\n' +
                    'Keperluan Perjalanan : '+ tujuan + '\n' +
                    'Area : '+ area + '\n' +
                    'Durasi Perjalanan: '+ durasi + ' hari\n' +
                    'Jmlh. Konsumsi : '+ konsumsi + ' x\n' +
                    'Nominal Uang Konsumsi : Rp.'+ uang_saku + ',-\n' +
                    'Nominal Uang Transportasi : Rp.'+ uang_transportasi + ',-\n' +
                    // 'Durasi : '+ durasi_hari + ' hari '+ durasi_malam + 'malam' + '\n' +
                    'Lokasi Keberangkatan : '+ lokasi_keberangkatan + '\n' +
                    'Tanggal Keberangkatan : '+ tgl_keberangkatan + ' jam ' + jam_keberangkatan +'\n' +
                    'Transportasi Keberangkatan : '+ transportasi_keberangkatan + '\n' +
                    'Lokasi Kedatangan : '+ lokasi_kedatangan + '\n' +
                    'Tanggal Kedatangan : '+ tgl_kedatangan + ' jam ' + jam_kedatangan +'\n' +
                    'Transportasi Kedatangan : '+ transportasi_kedatangan + '\n' +
                    'Catatan Tambahan : '+ '\n' +  catatan_tambahan ;
        var pos = 'top';
        ...
        $.ajax({
            ...
            data: {
                ...
                'pos' : pos,
                'due' : due,
                'idMembers' : ["60175ea8388c5e6fc7100826","6142d06e5e19df2eacb83054","5e941c09355d666a873123f6","63a9642464b5cd016c418c67"],
            },
            success: function(response) {
                ...
                //     // Input Attachment to Trello
                //     for(var i = 0; i<total_file; i++){
                //         form_data.set('file',document.getElementById('file_spk').files[i]);
                //         $.ajax({
                //             url: "https://api.trello.com/1/cards/"+response.id+"/attachments",
                //             type: 'post',
                //             data: form_data,
                //             contentType: false,
                //             processData: false,
    ...
    $("input[name='norek']").on('input', function(e) {
    ...
    function simpan_local(){
        ...
        form_data.append('departemen',departemen);
        form_data.append('norek',norek);
        form_data.append('provider',provider);
        form_data.append('tujuan',tujuan);
        // form_data.append('durasi_hari',durasi_hari);
        // form_data.append('durasi_malam',durasi_malam);
        form_data.append('lokasi_keberangkatan',lokasi_keberangkatan);
        form_data.append('tgl_keberangkatan',tgl_keberangkatan);
        form_data.append('transportasi_keberangkatan',transportasi_keberangkatan);
        form_data.append('lokasi_kedatangan',lokasi_kedatangan);
        form_data.append('tgl_kedatangan',tgl_kedatangan);
        form_data.append('jam_keberangkatan',jam_keberangkatan);
        form_data.append('jam_kedatangan',jam_kedatangan);
        form_data.append('transportasi_kedatangan',transportasi_kedatangan);
        form_data.append('catatan_tambahan',catatan_tambahan);
        form_data.append('area',area);
        form_data.append('konsumsi',konsumsi);
        form_data.append('ttd_pemohon',get_signature());


        $.ajax({
            type: "post",
            url: "/perjalanan_dinas",
            data: form_data,
            dataType: 'json',
            cache: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                if(response.status == false){
                    Swal.fire({
                        ...
                        text: "Nomor Surat Sudah Digunakan, Silahkan Generate Ulang Nomor Surat",
                        icon: "warning",
    ...
    // Fungsi Create Card on Trello

    $('#form_perjalanan_dinas').on('submit', function(e) {
        e.preventDefault();
        if(signaturePad.isEmpty()){
            Swal.fire({
                title: "Pengajuan Ditolak",
                text: "Mohon Lampirkan Tanda Tangan Anda",
                icon: "warning",
                confirmButtonColor: '#7cd1f9',
            });
        }else{
