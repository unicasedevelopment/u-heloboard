$(document).ready(function() {
    ...
    $('#form_edit_pengajuan_dana').on('submit', function(e) {
        ...
        var file_bukti_bayar = document.getElementById('file_fp_internal').files[0];
        form_data.append('file_fp_internal',file_bukti_bayar == undefined ? null:file_bukti_bayar);

        $.ajax({
            type: "post",
            url: "/validasi_pengajuan_dana/"+id_pengajuan,
            data: form_data,
            processData: false,
            contentType: false,
            dataType: 'JSON',
