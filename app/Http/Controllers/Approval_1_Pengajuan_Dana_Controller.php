...
class Approval_1_Pengajuan_Dana_Controller extends Controller
{
    ...
    public function index(){
        ...
        return view('data_pengajuan_dana.approval_1_pengajuan_dana.index',compact('data_pengajuan_dana','data_jenis_surat','data_divisi','data_kategori_dana','data_kategori','metode_pembayaran','jenis_pajak','no_surat'));
    }

    public function getData($status){
        if(auth()->user()->role == 'superadmin'){
            if($status !== 'approval_1'){
                if($status == 34){
                    ...
                    $data_pengajuan_dana = Pengajuan_Dana::where('status_pengajuan','<',6)->orWhere('status_pengajuan','=',7)->with('creator')->with('levels')->with('kategories')->get();
                    $cek = 'approval_1';
            }
        }else{
            ...
            if($status !== 'approval_1'){
                if($status == 34){
                    $data_pengajuan_dana = Pengajuan_Dana::where('approved_1_by',auth()->user()->id)->where('status_pengajuan','>', 2)->where('status_pengajuan','<',5)->with('creator')->with('levels')->with('kategories')->get();
                    $cek = '34';
                }else{
                    $data_pengajuan_dana = Pengajuan_Dana::where('approved_1_by',auth()->user()->id)->where('status_pengajuan',$status)->with('creator')->with('levels')->with('kategories')->get();
                    $cek = $status;
                }
            }else{
                    $data_pengajuan_dana = Pengajuan_Dana::where('approved_1_by',auth()->user()->id)->where('status_pengajuan','<',6)->orWhere('status_pengajuan','=',7)->with('creator')->with('levels')->with('kategories')->get();
                    $cek = 'approval_1';
            }
        }
        ...
        return response()->json([
            ...
            "status" => $status,
            "cek" => $cek,
            "total_data" => count($data_pengajuan_dana)
        ]);
