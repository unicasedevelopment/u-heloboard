...
class Approval_2_Pengajuan_Dana_Controller extends Controller
{
    /**
    ...
    public function index(){
        ...
        return view('data_pengajuan_dana.approval_2_pengajuan_dana.index',compact('data_pengajuan_dana','data_jenis_surat','data_divisi','data_kategori_dana','data_kategori','metode_pembayaran','jenis_pajak','no_surat'));
    }

    public function getData($status){
        if($status !== 'approval_2'){
            if($status == 34){
            ...
            $data_pengajuan_dana = Pengajuan_Dana::where('status_pengajuan','<',6)->where('status_pengajuan','>',1)->orWhere('status_pengajuan','=',7)->with('creator')->with('levels')->with('kategories')->get();
        }
    ...
    public function cetak($id){
        $data_pengajuan_dana = Pengajuan_Dana::find($id)->with('creator')->with('levels')->with('kategories')->with('dokumen_pengajuans')->with('informasi_lampirans')->with('informasi_pembayarans')->with('informasi_perpajakans')->get();
        $status = false;
        if(count($data_pengajuan_dana) > 0){
            if($data_pengajuan_dana[0]->status_validasi == 'yes'){
                $status = true;
            }
        }
        return response()->json([
            'status' => $status,
            'data' => $data_pengajuan_dana[0]
        ]);
    }
}